...
class SemiEmpricalQuatumMechanicalEnergyEvaluator(EnergyEvaluator):
    def __init__(self, ob_mol, ob_fragments, nums_fragments, total_charge,
        ...
        self.contact_detector = self._construct_contact_detector(
            ...
            self.mol_coords, ob_mol, ob_fragments, nums_fragments)
        self.layout_order = OrderredLayoutEnergyEvaluator(self.mol_coords,
                                                          nums_fragments)
        grav = "rmsd_gap"
        if grav == "rmsd_gap":
        ...
        self.fragments_species = [IonPlacer.get_mol_species(frag) for frag in
                                  ob_fragments]
        ...
        self.sqm_umbrella = UmbrellarForceEnergyEvaluator(self.mol_coords,
                                                          self.sqm_umbrella_centers,
    ...
    def save_current_best_position(self):
        ...
        mol_file_name = os.path.join(conformer_dir,
                                     "step_{}_energy_{}.xyz".format(
                                         self.best_run_number,
                                         self.best_energy))
        BabelMolAdaptor(self.best_mol).write_file(mol_file_name,
                                                  file_format="xyz")
    ...
    def append_position_to_memory(self, fragments_coords, energy):
        self.memory_positions.insert(0, tuple(
            [list(itertools.chain(*fragments_coords)), energy]))
    ...
    def is_optimized_position_inside_the_best_promixity(self,
                                                        fragments_coords):
    ...
    def build_or_extend_gap_umbrella(self, current_gap, fragments_coords):
        ...
        if self.gap_umbrella is None:
            self.gap_umbrella = UmbrellarForceEnergyEvaluator(self.mol_coords,
                                                              self.gap_umbrella_centers,
                                                              self.taboo_tolerance_au,
                                                              self.best_umbrella_ratio)
    ...
    def _get_super_molecule(self, fragments_coords):
        ...
        for frag_sp, num_frag in zip(self.fragments_species,
                                     self.nums_fragments):
        ...
        super_mol_coords_ang = [
            [x / AtomicRadiusUtils.angstrom2au for x in coord] for coord in
            super_mol_coords_au]
        return Molecule(super_mol_species, super_mol_coords_ang)

    @staticmethod
    def _construct_hardsphere_energy_evaluator(covalent_radius_scale,
                                               metal_radius_scale,
                                               mol_coords, ob_mol,
                                               ob_fragments, nums_fragments):
    ...
    def _construct_contact_detector(covalent_radius_scale, metal_radius_scale,
                                    mol_coords, ob_mol, ob_fragments,
                                    nums_fragments):
        rad_util = AtomicRadiusUtils(covalent_radius_scale, metal_radius_scale)
        mol_radius = rad_util.get_radius(ob_mol)
        fragments_atom_radius = [rad_util.get_radius(frag) for frag in
                                 ob_fragments]
        detector = ContactDetector(mol_coords, mol_radius,
    ...
    def _construct_largest_cap_energy_evaluator(covalent_radius_scale,
        ...
        evaluator = LargestContactGapEnergyEvaluator(
            mol_coords, mol_radius, fragments_atom_radius, nums_fragments,
            max_cap, threshold=0.01)
        return evaluator

    @staticmethod
    def _construct_contact_cap_rmsd_energy_evaluator(covalent_radius_scale,
                                                     metal_radius_scale,
                                                     mol_coords, ob_mol,
                                                     ob_fragments,
                                                     nums_fragments):
        ...
        fragments_atom_radius = [rad_util.get_radius(frag) for frag in
                                 ob_fragments]
        evaluator = ContactGapRMSDEnergyEvaluator(mol_coords, mol_radius,
